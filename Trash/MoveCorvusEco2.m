classdef MoveCorvusEco2 < handle
    ...
    methods
        ...
        %% constructor
        function self = MoveCorvusEco2()
            self.Name = 'CorvusEco'; % name of the instrument
            self.Connected = 1;
            % instrument parameters
            self.Param.COMPort = 7;
...
%                     response = self.read_response();
        ...
        function self = send_command(self, command)
            disp 'in send command function'
            if self.Obj.BytesAvailable > 0  %empty buffer
                disp 'BytesAvailable > 0'
        ...
        function self = wait_for_command(self)
            ...
            while (~self.Obj.BytesAvailable && toc(start_time) < self.Timeout)
                pause(self.PauseTime);
                %disp('waiting for motor controller');
            end
        ...
        function [a1, a2] = set_closed_loop(self, toggle)
           %enables the closed loop control for axis 1 and 2; 
           %if toggle=1: enable closed loop if toggle=0: disable
           % returns setting in stage controller for axis 1 and 2; s
           if self.Connected
               self.Busy = 1;
               
               %[indx][axis] setcloop
               send_command(strcat(num2str(toggle),' 1 setcloop')); %change axis 1
               send_command(strcat(num2str(toggle),' 2 setcloop')); %change axis 2
               %Read settings back 
               ...
               err = MException(strcat(self.Name,':setcloop'),...
                   'setcloop: optical stage not connected');
               self.Busy=0;
               throw(err);
           end
        ...
        function self = move_y(self, distance)
            if self.Connected
                ...
                self.wait_for_command();
%                 st = self.read_response();
%                 if str2num(st)
%                     disp(strcat('status retunred from motor: ', st));
%                     err = MException(strcat(self.Name,':MoveY'),...
%                         'move_y: controller returned error');
%                     self.Busy=0;
%                     throw(err);
%                 end
%                 self.yPos = self.yPos + distance/1000;
                self.Busy = 0;
                pause(0.01);
            else
                err = MException(strcat(self.Name,':MoveY'),...
        ...
        function self = move_x(self, distance)
            ...
            if self.Connected
                %                 if self.Calibrated
                self.Busy = 1;
                move_cmd = [num2str(distance/1000), ' 0 r']  %stage takes mm. the classes have um.
                end_cmd = '0 0 r';
                stop_cmd = 'st';
                self.send_command(move_cmd);
                disp('moving x')
...
%                         'move_x: controller returned error');
%                     self.Busy=0;
%                     throw(err);
%                 end
        ...
        function self = moveRelative(self, distance)
            if self.Connected
                self.Busy = 1;
                move_cmd = [num2str(distance/1000),' r']
                
%                 move_cmd = sprintf('%.6f r',distance)
                self.send_command(move_cmd);
                self.Busy=0;
                
            else
                err = MException(strcat(self.Name,':MoveRelative'),...
                    'unable to make a relative move');
                self.Busy=0;
                throw(err);
                
            end
        end
        function self = move_try(self, distance)
            distance
            disp 'in function move try'
            if self.Connected
                %                 if self.Calibrated
                self.Busy = 1;
                move_cmd = [num2str(distance/1000), ' 0 rmove']; %stage takes mm. the classes have um.
                end_cmd = '0 0 r';
                stop_cmd = 'st';
                self.send_command('10 r');
                disp('moving x')
%                 self.send_command(end_cmd);
%                 self.send_command(stop_cmd);
%                 self.wait_for_command();
...
%                 end
        ...
        function self = moveTo(self, x, y)
            ...
            if self.coordSysIsValid
                ...
                msg = strcat(self.Name, ': moving to ', num2str(x), ',', num2str(y));
                disp(msg);
...
%         %% get all parameters (overloads super class method
%         function val = getAllParams(self)
%             val = self.Param;
%             val.xmotpos = self.xPos;
%             val.ymotpos = self.yPos;
%             val.zmotpos = self.zPos; 
%         end
%         
%         
